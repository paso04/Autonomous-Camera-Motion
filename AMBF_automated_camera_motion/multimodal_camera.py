...
def PSM_cb(msg):
        ...
        if sensor2.get_sensed_object(0) == 'Needle':
                if min_dist > 0.15:
                        ...
                        fnp = PyKDL.Frame(T_c_w.M, PyKDL.Vector((V_RPY_2[0]+min_entry[0])/2,(V_RPY_2[1]+min_entry[1])/2 + 0.763076055,(V_RPY_2[2]+min_entry[2])/2 + 1.0712601185))
                        # ecm.servo_cp(fnp)
                        f=fnp
                else:
                        ...
                        # ambf_cam_l.set_pose_changed()
                        # ambf_cam_r.set_pose_changed()
                        ftb = PyKDL.Frame(T_c_w.M, PyKDL.Vector(min_entry[0]+0.05,min_entry[1]+0.763076055,min_entry[2]+0.9712601185))
                        # ecm.servo_cp(ftb)
                        f=ftb
                ecm.servo_cp(f)
                ...
                if min_dist > 0.15:
                        ...
                        V_RPY_2 = c.get_obj_pose('/ambf/env/psm2/toolyawlink')
                        fps = PyKDL.Frame(T_c_w.M, PyKDL.Vector((V_RPY_2[0]+V_RPY_1[0])/2,(V_RPY_2[1]+V_RPY_1[1])/2 + 0.763076055,(V_RPY_2[2]+V_RPY_1[2])/2 + 0.8712601185))
                        # ecm.servo_cp(fps)
                        f=fps
